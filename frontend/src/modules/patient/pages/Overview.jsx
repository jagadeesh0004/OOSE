...
import { Ico, PageLoader, Spinner, toast, Empty } from "../../../components/common/Toast";
import { IC } from "../../../utils/constants";
import { StatusBadge, RiskBadge, StatCard, SectionCard } from "../../../components/common/UI";

export function PatientOverview({ user, onNav }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [a, p] = await Promise.allSettled([api("/appointments/my-appointments/"), api("/predictions/history/")]);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      >
        ...
        <StatCard
          ...
          sub={
            lastPred
              ? new Date(lastPred.created_at).toLocaleDateString()
              : "No prediction yet"
          }
          delay={180}
        />
      ...
      <SectionCard title="Upcoming Appointments" action={<button className="cta-ghost" onClick={() => onNav("appointments")} style={{ padding: "7px 14px", fontSize: 12 }}>View All</button>} noPad accent="#0ea5e9">
        {upcoming.length === 0 ? (
      ...
      {/* Last prediction */}
      {lastPred && (
        <SectionCard accent="#8b5cf6" noPad>
