...
// ─────────────────────────────────────────────────────────────────────────────
// InfoRow — Reusable field row component (memoized to prevent remount)
// ─────────────────────────────────────────────────────────────────────────────
function InfoRow({ label, value, editKey, type, form, up, editing }) {
  return (
    <Field label={label}>
      {editing
...
export function Profile({ doctor, onUpdate }) {
  ...
  const updSlot = useCallback((i, k, v) => {
    setForm((p) => ({
      ...p,
      time_slots: (p.time_slots || []).map((s, j) => (j === i ? { ...s, [k]: v } : s))
    }));
  }, []);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18, animation: "fadeUp 0.4s ease" }}>
      ...
      <div className="feature-card" style={{ "--accent": "linear-gradient(135deg,#0ea5e9,#0284c7)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 18 }}>
          <InfoRow label="Specialization"               value={doctor.specialization}        editKey="specialization" type="text" form={form} up={up} editing={editing} />
          <InfoRow label="Qualification"                value={doctor.qualification}         editKey="qualification" type="text" form={form} up={up} editing={editing} />
          <InfoRow label="Experience (years)"           value={doctor.experience_years}      editKey="experience_years" type="number" form={form} up={up} editing={editing} />
          <InfoRow label="Consultation Fee (₹)"         value={doctor.consultation_fee}      editKey="consultation_fee" type="number" form={form} up={up} editing={editing} />
          <InfoRow label="Consultation Duration (min)"  value={doctor.consultation_duration} editKey="consultation_duration" type="number" form={form} up={up} editing={editing} />
          <InfoRow label="Slot Duration (min)"          value={doctor.slot_duration}         editKey="slot_duration" type="number" form={form} up={up} editing={editing} />
        </div>
      </div>
      ...
      <div className="feature-card" style={{ "--accent": "linear-gradient(135deg,#8b5cf6,#6d28d9)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 18 }}>
          <InfoRow label="Hospital Name" value={doctor.hospital_name} editKey="hospital_name" type="text" form={form} up={up} editing={editing} />
